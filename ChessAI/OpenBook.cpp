...
std::vector<yunkuResult> Yunku::calcSteps(std::string fen)
{
    ...
    {
        std::string move, score, winrate;

        std::string result = results[i];
        int idx1, idx2;
        idx1 = result.find("move");
        idx2 = result.find(",",idx1);
        if (idx1!=-1 && idx2 !=-1)
        {
            move = result.substr(idx1 + 5, idx2 - (idx1+5));

            idx1 = result.find("score");
            idx2 = result.find(",", idx1);
            if (idx1 != -1 && idx2 != -1)
            {
                score = result.substr(idx1 + 6, idx2 - (idx1 + 6));

                idx1 = result.find("winrate");
                if (idx1 !=-1)
                {
                    winrate = result.substr(idx1 + 8);

                    retVec.push_back(yunkuResult(move, score, winrate));
                }
            }
        }
    }
...
std::vector<openbookResult> OpenBook::calcSteps(std::string fen)
{
    ...
    std::string sql = "SELECT * FROM bhobk WHERE cast(vkey as integer) = " + std::to_string(key);
    if (key >> 63 != 0)
    {
        double* keyPtr = (double*)&key;
        double keyDbl = *keyPtr;
        ...
        oss << std::setprecision(std::numeric_limits<double>::digits10 + 1) << keyDbl;
        std::string str_value = oss.str();
        sql = "SELECT * FROM bhobk WHERE cast(vkey as text) = " + str_value; //std::to_string(keyDbl)这个不行
    }
    std::vector<std::map<std::string, std::string>> result = qSql.query(sql);
    for (size_t i = 0; i < result.size(); i++)
    {
        std::string vmove = result[i]["vmove"];
        std::string move = ConvertVmoveToCoord(atoi(vmove.c_str()), false);
        printf("move: %s\n", move.c_str());
        std::string vscore = result[i]["vscore"];
        std::string vwin = result[i]["vwin"];
        std::string vdraw = result[i]["vdraw"];
        std::string vlost = result[i]["vlost"];
        std::string vvalid = result[i]["vvalid"];
        std::string vmemo = Utils::utf8_to_ansi(result[i]["vmemo"]);
        retVec.push_back(openbookResult(move, vscore, vwin, vdraw, vlost, vvalid, vmemo));
    }


    key = GetZobristFromFen(fen,true);
    sql = "SELECT * FROM bhobk WHERE cast(vkey as integer) = " + std::to_string(key);
    if (key >> 63 != 0)
    {
        double* keyPtr = (double*)&key;
        double keyDbl = *keyPtr;
    ...
    std::vector<std::map<std::string, std::string>> result_leftRightSwap = qSql.query(sql);

    for (size_t i = 0; i < result_leftRightSwap.size(); i++)
    {
        std::string vmove = result_leftRightSwap[i]["vmove"];
        std::string move = ConvertVmoveToCoord(atoi(vmove.c_str()), true);
        printf("move: %s\n", move.c_str());
        std::string vscore = result_leftRightSwap[i]["vscore"];
        std::string vwin = result_leftRightSwap[i]["vwin"];
        std::string vdraw = result_leftRightSwap[i]["vdraw"];
        std::string vlost = result_leftRightSwap[i]["vlost"];
        std::string vvalid = result_leftRightSwap[i]["vvalid"];
        std::string vmemo = Utils::utf8_to_ansi(result_leftRightSwap[i]["vmemo"]);
        retVec.push_back(openbookResult(move, vscore, vwin, vdraw, vlost, vvalid, vmemo));
    }
...
std::string OpenBook::ConvertVmoveToCoord(int vmove, bool swapLeftRight)
{
	...
	int to = vmove % 256;
    if (swapLeftRight) {
        from = from & ~15 | (14 - from % 16);
        to = to & ~15 | (14 - to % 16);
    }
	std::string coord = CoordMap[from] + CoordMap[to];
	return coord;
}

int64_t OpenBook::GetZobristFromBoard(char board[10][9], char side,bool swapLeftRight)
{
    int64_t ZobristLock = 0;
    for (int y = 0; y < 10; y++)
    {
        for (int x = 0; x < 9; x++)
        {
            ...
            if (pieces.count(letter) > 0)
            {
                int chess = pieces[letter];
                int c90_index = (swapLeftRight ? (8-x) : x) + y * 9;
                ...
                printf("c90_index:%d letter:%c chess:%d square:%d\n", c90_index, letter, chess, square);
...
int64_t OpenBook::GetZobristFromFen(std::string fen, bool swapLeftRight)
{
    fen = Utils::trim(fen);
    char board[10][9] = {
        {' ', ' ', ' ',' ',' ',' ', ' ', ' ', ' '}, //0
    ...
    std::string base_fen = fen.substr(0, fen.find_first_of(' '));

    //fen to maps
    std::vector<std::string> rows = Utils::splitStr(base_fen, "/");
    ...
    return GetZobristFromBoard(board, side ,swapLeftRight);
}
...
OpenBook::OpenBook()
{
   ...
   ZobristPlayer = 0xA0CE2AF90C452F58;
   int64_t ZobristTableTmp[3584] =
    {
    ...
    {
        this->ZobristTable[i] = ZobristTableTmp[i];
    }

    for (int i = 0; i < 90; i++)
    {
        int key = c90[i];
        std::string value = san90[i];
        CoordMap.insert(std::make_pair(key, value));
    }
}
